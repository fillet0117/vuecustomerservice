<template>
  ...
  >
    ...
    <el-form-item :label="$t('views.main.photo')">
      ...
      <el-popover v-if="upfile2 !== '' && upfile === ''">
        <img slot="reference" :src="upfile2" style="width: 100px; height: 100px" />
      </el-popover>
    </el-form-item>
    <br />
    <el-form-item>
...
<script>
import { editmanager } from "../../../api/chat";
export default {
  props: ["data"],
  data() {
    return {
  ...
  mounted() {
    ...
    this.form.level = this.data.le;
    this.form.status = this.data.st == "1" ? true : false;
    this.upfile2 = this.data.photo;
  },
  methods: {
    onSubmit() {
      var _this = this;
